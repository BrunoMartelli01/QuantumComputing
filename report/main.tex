...
\LH{Moreover, the use of different types of QPUs enables experimentation within the domain of Quantum High Performance Computing (QHPC).} \LH{(Possibile alternativa alla frase precedente: ``We see a single pipeline integrating the use of gate-based and quantum-annealing based QPUs as a concrete contribution to the area of Quantum High Performance Computing (QHPC)''?)}
...
%In this section, we briefly recall the theoretical foundations underlying the construction of a gate-based quantum circuit employed to implement a quantum kernel.
%
%\LH{(1)} The use of quantum kernels enables the mapping of input examples into a hyperspace of dimension $O(2^{\#\text{features}})$.
%This property stems from the fact that states formed by qubits can be superposed during a computation.
%
%\LH{(2)} Quantum kernel \cite{qkernel} methods are based on the construction of a quantum circuit that is executed for each pair of input data points.
%
%\LH{(3)} Specifically, given two classical input vectors $\vec{x}_i$ and $\vec{x}_j$, the quantum kernel evaluates the similarity between them by computing the fidelity (i.e., squared inner product) of their corresponding quantum states in Hilbert space.
%
%\LH{(4)}  The typical procedure involves the following steps:
%\begin{enumerate*}
...
\begin{enumerate}
  \item \LH{(È corretto??)} Each $\vec{x}\in V$ is interpreted as a quantum state $\ket{\phi(\vec{x})}$ by means of a quantum circuit $\phi(\vec{x})$;
...
\end{enumerate}
\noindent
This procedure allows the definition of a kernel matrix $K$. Each entry $K_{ij}$ corresponds to the \LM{similarity} between  $\vec{x}_i$ and $\vec{x}_j$ \LH{``measured'' after their representation as quantum states $\ket{\phi(\vec{x}_i)}$ and $\ket{\phi(\vec{x}_j)}$}.

Intuitively, the speed-up to construct $K$ of the above method, compared to equivalent classical algorithms, follows from superposing -- hence ``compressing'' -- a vector space, potentially with dimension $O(2^{\#\text{features}})$.

\LHs{Quantum kernel \cite{qkernel} methods are based on the construction of a quantum circuit that is executed for each pair of input data points.}

\LHs{The use of quantum kernels enables the mapping of input examples into a hyperspace of dimension $O(2^{\#\text{features}})$. This property stems from the fact that states formed by qubits can be superposed during a computation.}
% LUCA ================================
...
An additional hyperparameter of practical importance is a \emph{regularization parameter} $C$. It controls the trade-off between maximizing the margin \LH{(Magari è terminologia standard, ma può aiutare dire ``margin di cosa''?)} and minimizing classification errors.
...
Both the design of a kernel and the optimization of the SVM dual \LH{(In questo contesto vale la pena specificare che è il problema duale? ``dual'' l'hai usata anche prima)} problem require the selection of hyperparameters prior to training.
...
Once the optimal hyperparameters for the individual components of the problem have been identified, Section \ref{sec:qkernel_and_qasvm} presents the results of the end-to-end learning pipeline for fully quantum SVMs—namely, those employing a kernel generated via quantum gates and optimization performed through quantum annealing.
...
\LH{Sono arrivato sin qui. Ora devo dare retta a Pensa... Forse riprendo stasera}
