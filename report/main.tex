...
\end{enumerate}
This procedure allows the definition of a kernel matrix $K$. Each entry $K_{ij}$ corresponds to the similarity between  $\vec{x}_i$ and $\vec{x}_j$ as inferred from their representation as quantum states $\ket{\phi(\vec{x}_i)}$ and $\ket{\phi(\vec{x}_j)}$.

Intuitively, the advantage in constructing $K$ with the above method, compared to equivalent classical algorithms, follows from superposing — hence “compressing” — a vector space, potentially with dimension $O(2^\text{\#features})$, which enhances the expressive power of the resulting kernel. \LH{(Cosa si intende per ``expressive power''?)}
...
Section \ref{sec:qkernel} presents our investigation into the parametrization of the \LH{contruction of a(?)} quantum kernel \LH{in the context of gate-based architectures,} with respect to \LHs{the} \LH{a(?)} chosen dataset.
Section \ref{sec:qasvm} reports on a set of experiments aimed at selecting the parameter $C$ \LH{in the context of quantum-anneal based architectures,} assuming no kernel is employed during training.

Once the optimal hyperparameters for the individual components of the problem have been \LH{indipendently} identified, Section \ref{sec:qkernel_and_qasvm} presents the results of the end-to-end learning pipeline for fully quantum SVMs—namely, those employing a kernel generated via quantum gates and optimization performed through quantum annealing.
...
\begin{description}
  ...
  Defines how many times the feature map circuit is applied in sequence \LH{(Esiste un possibile commento che renda intuitivo perché le features map si devono poter essere concatenate? Non basta una singola feature map per ottenere quello che prima abbiamo chiamato $\phi(\vec{x})$? Cioè una singola feature map non è $\phi$? Dalla Tabella sembra che la risposta è: ``in certi casi no''. Corretto?)}. Increasing repetitions enhances the circuit's expressiveness but also deepens the circuit, which may lead to increased noise on real hardware.
\end{description}
\noindent
...
In our experiments, we explored a range of values for $C$ by selecting powers of two, from $2^2 - 1$ to $2^{12} - 1$. \LH{(Non mi è chiarissima la comunità di quelli che potrebbero leggere queste pagine. Se lo leggesse uno come me, avrebbe difficoltà nell'essere sicuro che quanto hai detto nella frase precedente significa: ``ho fissato una lunghezza $l$ potenza di 2, e ho preso vettori a caso di quella lunghezza per imparare la SVN''. Quanto è sbagliato? Si può aggiungere una contestualizzazione come quella appena suggerita, ovviamente corretta?)}
...
By combining quantum kernels with optimization via quantum annealing, we constructed several QSVM models \LH{(Che imparano cosa? Sono costruiti in analogia a benchmark tipici del machine learning? Altro?)}, obtaining a wide range of results.

The best-performing model was based on the kernel with the highest alignment (using 30 qubits) \LH{(È ovvio in questo contesto cosa sia un allineamento basato su 30 qbit? Nel mondo classico gli allineamenti sono basati su bit?)} and a regularization parameter $C=255$.
...
The best-performing configuration, employing 30 qubits and a regularization parameter $C=255$, achieved an F1-score of 90\%.
However, the presence of systematic false negatives suggests that further improvements could be achieved with a more refined data preprocessing pipeline. \LH{(Può dipendere anche dal conoscere la natura/provenienza del training/test set, cioè dall'ambito che genera i dati da cui apprendere?)}

This work \LHs{opens the door for} \LH{is a first step towards} future investigations into scalable quantum machine learning pipelines and highlights the role of quantum high-performance computing in facilitating hybrid computational workflows across diverse quantum architectures.
