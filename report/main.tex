...
\begin{description}
  ...
  Figure~\ref{fig:map_a}, Figure~\ref{fig:map_b} and Figure~\ref{fig:map_c} \LHs{shows a couple} \LH{are three examples} of possible feature maps.
...
By combining quantum kernels with optimization via quantum annealing, we constructed several QSVM models, aimed at learning decision boundaries that separate data classes in a feature space implicitly defined by the quantum kernel.
...
\LH{As usual(?), } a more effective preprocessing phase would likely result in a uniform performance improvement across all configurations.
