...
\LM{Intuitively, the advantage of constructing the matrix $K$, which represents the similarities between examples, using the method described above, lies in the ability of quantum kernels to exploit qubit entanglement and state superposition. 
This allows for a compression in the number of qubits required, as adding a single qubit to the feature map $\phi$ doubles the dimensionality of the space to which the input $x_i$ is mapped.}
...
\begin{description}
  ...
  Figure~\ref{fig:map_a}, Figure~\ref{fig:map_b} and Figure~\ref{fig:map_c} shows a couple of possible feature maps.

  \item[Repetitions of the feature map:]
  Defines how many times the feature map circuit is applied in sequence.
  \LM{The reason repetitions of the same feature map may be required is that, in certain cases, a single application is not sufficient to capture the desired non-linear transformation $\phi(\vec{x})$. 
...
\LM{In our experiments, we explored a range of values for $C$ by selecting powers of two, from $2^2 - 1$ to $2^{12} - 1$. 
...
\LM{By combining quantum kernels with optimization via quantum annealing, we constructed several QSVM models, aimed at learning decision boundaries that separate data classes in a feature space implicitly defined by the quantum kernel. 
...
\LM{While incorporating domain knowledge about the origin and nature of the training and test datasets could also be beneficial, we believe that the most effective improvements are likely to come from enhanced preprocessing strategies or the use of larger and more representative datasets. 
