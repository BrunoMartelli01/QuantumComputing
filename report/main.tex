...
\end{enumerate}
This procedure allows the definition of a kernel matrix $K$. Each entry $K_{ij}$ corresponds to the similarity between  $\vec{x}_i$ and $\vec{x}_j$ \LM{as inferred from} their representation as quantum states $\ket{\phi(\vec{x}_i)}$ and $\ket{\phi(\vec{x}_j)}$.

Intuitively, the \LM{advantage} in constructing $K$ with the above method, compared to equivalent classical algorithms, follows from superposing — hence “compressing” — a vector space, potentially with dimension $O(2^\text{\#features})$, \LM{which enhances the expressive power of the resulting kernel.}
...
\LM{In this context, the margin refers to the distance between the separating hyperplane and the closest data points from each class.}
